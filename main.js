...
(function() {
  ...
  const API_KEY = "?api_key=RGAPI-f74bd49f-d4bd-448d-a5df-3ebcfc0d1bff";
  ...
  function initialize() {
    ...
    document.getElementById("search-button").addEventListener("click", function(){
      ...
      makeRequest(URL + YORHA_SUMMONERS_BY_NAME + summonerName, createSummonerPage, checkStatusJSON);
    });
  ...
  function createSummonerPage(responseData) {
    ...
    makeRequest(URL + YORHA_MASTERY + responseData["id"],
  ...
  function loadMastery(responseData) {
    ...
    // preload first 20 matches and set to hidden?
    matchHistoryButton.addEventListener("click", function() {
      makeRequest(URL + YORHA_MATCHLIST + pageOwnerId,
  ...
  function createMatch(i, matches, matchHistoryBox) {
    ...
    makeRequest(URL + YORHA_MATCH + match["gameId"],
